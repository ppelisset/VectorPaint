...
public class Rectangle extends Figure {
	...
	/**
	 * Methode de construction du rectangle
	 * 
	 * @param v
	 */
	...
	/**
	 * Retourne le vecteur créateur du rectangle
	 * 
	 * @return Vector
	 */
	...
	/**
	 * Modifie le vecteru createur du rectangle
	 * 
	 * @param v
	 */
	...
	public void resize(double originTop, double originLeft, double endTop,
			double endLeft) {
		if (Math.abs(originTop - _vector.getTopDistance()) < Math.abs(originTop
				- _vector.getEndTopDistance())
				&& Math.abs(originTop - _vector.getTopDistance()) < Rectangle.ERROR_MARGE) {
		...
		if (Math.abs(originLeft - _vector.getLeftDistance()) < Math
				.abs(originLeft - _vector.getEndLeftDistance())
				&& Math.abs(originLeft - _vector.getLeftDistance()) < Rectangle.ERROR_MARGE) {
		...
		if (_vector.getEndLeftDistance() - _vector.getLeftDistance() < 1
				|| _vector.getEndTopDistance() - _vector.getTopDistance() < 1) {
			double tmp;
			if (this._vector.getTopDistance() > this._vector
					.getEndTopDistance()) {
				tmp = _vector.getTopDistance();
				_vector.setTopDistance(_vector.getEndTopDistance());
				_vector.setEndTopDistance(tmp);
			}
			if (this._vector.getLeftDistance() > this._vector
					.getEndLeftDistance()) {
	...
	/**
	 * Methode toStriong du Rectangle
	 */
	...
	@Override
	public String save() {
		return "[" + _vector.save() + ";" + (isFill() ? 1 : 0) + ";"
				+ Figure.encodeColor(getColor()) + "]";
	}

	/**
	 * Methode de création du Rectangle depuis lecture d'un fichier .vpf
	 * 
	 * @param s
	 * @return
